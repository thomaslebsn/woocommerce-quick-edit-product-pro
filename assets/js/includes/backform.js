...
(function(root, factory) {
  ...
  */
  ...
  _.extend(JSONFormatter.prototype, {
    fromRaw: function(rawData, model) {
      return _.isArray(rawData) ? JSON.stringify(rawData) : rawData;
    },
    toRaw: function(formattedData, model) {
      return _.isJSON(formattedData) ? JSON.parse(formattedData) : formattedData;
    }
  });
  ...
  var Control = Backform.Control = Backbone.View.extend({
    ...
    onChange: function(e) {
        // todo : find out how to custom this event to multiple checkbox on change within list.
  ...
  var BooleanControl = Backform.BooleanControl = InputControl.extend({
    ...
    template: _.template([
      '<label class="<%=Backform.controlLabelClassName%>"><%=controlLabel%>',
      '    <% if(required) { %>',
      '        <span class="required-field">*</span>',
      '    <% } %>',
      '</label>',
